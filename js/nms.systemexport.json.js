function ExportData() {
    ...
    for (var i = 1; i <= SystemCount; i++) { 
        ...
        for (var j = 1; j < PlanetCount; j++) {
            ...
            var grassNums = []
            for (var g = 0; g < grass.length; g++){ grassNums.push(grass[g].innerText);}
            var plant = PlanetTable.rows[15].cells[j].getElementsByTagName("table")[0].rows[0].cells;
            var plantNums = []
            for (var g = 0; g < plant.length; g++){ plantNums.push(plant[g].innerText);}
            var leaf = PlanetTable.rows[16].cells[j].getElementsByTagName("table")[0].rows[0].cells;
            var leafNums = []
            for (var g = 0; g < leaf.length; g++){ leafNums.push(leaf[g].innerText);}
            var tile = PlanetTable.rows[18].cells[j].getElementsByTagName("table")[0].rows[0].cells;
            var tileNums = []
            for (var g = 0; g < tile.length; g++){ tileNums.push(tile[g].innerText);}
            ...
            var planet = {
                ...
                Water: PlanetTable.rows[20].cells[j].innerText,
                ...
                Prime: PlanetTable.rows[27].cells[j].innerText,
                ForceContinents: PlanetTable.rows[28].cells[j].innerText,
                Rings: PlanetTable.rows[29].cells[j].innerText,
                Sandworms: PlanetTable.rows[30].cells[j].innerText,
                CreatureNumber: PlanetTable.rows[31].cells[j].innerText,
                SkyColour: PlanetTable.rows[17].cells[j].innerText,
            ...
            for (var k = 0; k < PlanetCreatureTable.length; k++) {
                ...
                for (var l = 0; l < PlanetCreatureSubTable.rows[1].cells.length; l++) {
                    var creature = {
                        ...
                        Weight:PlanetCreatureSubTable.rows[3].cells[l].innerText.split("/")[1],
                        Height:PlanetCreatureSubTable.rows[3].cells[l].innerText.split("/")[2],
                        Density: PlanetCreatureSubTable.rows[4].cells[l].innerText.split("/")[0],
                        GroupSize: PlanetCreatureSubTable.rows[4].cells[l].innerText.split("/")[1],
                        GroupsKm2: PlanetCreatureSubTable.rows[4].cells[l].innerText.split("/")[2],
                        Rarity: PlanetCreatureSubTable.rows[6].cells[l].innerText.split("/")[0],
                        Probability: PlanetCreatureSubTable.rows[6].cells[l].innerText.split("/")[1],
